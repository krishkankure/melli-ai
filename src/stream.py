...
def main():
    ...
    st.title("Melli - An AI Diabetes Predictor")
...
def check(pr, ag, gl, b, sk, i, w, h, dp):
    if (check_float(pr) and check_float(ag) and check_float(gl) and check_float(b) and check_float(sk) and check_float(
        ...
        if print_prob >= 90:
            ...
            amt = "somewhat"
        with st.spinner('Refreshing Learning Model...'):
            time.sleep(1.5)
        ...
        with st.spinner('Making Prediction...'):
            time.sleep(2)
        st.header("It is " + amt + " " + result + " that the patient will develop diabetes within the next 5 years.")
        st.write("It is " + str(print_prob) + "% probable that the patient will develop diabetes within the next 5 years.")
        if int(gl) > 200 or int(i) > 300:
            st.caption("Note: The information you provided indicates you may already have diabetes. [Learn more](https://www.cdc.gov/diabetes/basics/diabetes.html)")
    else:
